...
def weighted_cosine_similarity(k_lst1, k_lst2, model, threshold=0.85):
    sim_scores = []
...
for channel, keywords in keyword_data.items():
    keyword_data[channel] = keywords[:15]
data = {
    "YouTuber": [], #YouTuber
...
for index1, row1 in df.iterrows():
    ...
    for index2, row2 in df.iterrows():
        if index1 < index2:
            score = weighted_cosine_similarity(row1["keywords"], row2["keywords"], word2vec_model)
            if score > 0:
...
nx.write_gexf(word2vec_graph, "graph.gexf")
